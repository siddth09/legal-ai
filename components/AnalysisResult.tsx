...
import { CheckSquare, Copy, Check, Scale, Download } from "lucide-react";
...
function AnalysisResultInner({ analysis }: AnalysisResultProps) {
  ...
  const exportSummary = useCallback(() => {
    if (!analysis) return;
    const lines: string[] = [
      `LexAI Analysis — ${analysis.documentType}`,
      "=".repeat(50),
      "",
      "SUMMARY",
      analysis.summary,
      "",
      "RED FLAGS",
      ...(analysis.redFlags.length > 0 ? analysis.redFlags.map((f) => `• ${f}`) : ["None identified"]),
      "",
      "NEXT STEPS CHECKLIST",
      ...analysis.checklist.map((item, i) => `${i + 1}. ${item}`),
      "",
      "QUESTIONS FOR YOUR LAWYER",
      ...analysis.lawyerQuestions.map((q, i) => `${i + 1}. ${q}`),
      "",
      "---",
      "Generated by LexAI — AI-powered legal information only, not legal advice.",
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `lexai-summary-${Date.now()}.txt`;
    a.click();
  ...
  const tabs: { id: Tab; label: string; count?: number }[] = useMemo(() => [
    ...
    { id: "clauses",     label: "Clauses",     count: analysis?.clauses.length ?? 0 },
      ...
      <div className="card" style={{ padding: 20 }}>
        <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
          ...
          <button
            ...
            className="btn-ghost"
            style={{ flexShrink: 0, fontSize: 12, padding: "6px 10px", borderRadius: 8 }}
            aria-label="Export full analysis as text file"
...
export default memo(AnalysisResultInner);
